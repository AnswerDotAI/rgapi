...
- `nbrg(pattern, root=".", cell_context=0, maxlen=120, ...)` searches Jupyter `.ipynb` files (cell source only) and returns matched cells as `NbResults`/`NbCell`; `paths=True` and `count=True` reduce like `rg`'s. Its display is always a one-line cell summary. Use this for notebooks rather than `rg`, to avoid escaped JSON and get stable cell ids.
